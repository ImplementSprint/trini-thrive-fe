...
export default function VolunteerPage() {
  ...
  const selectedCampaign = volunteerCampaigns.find((campaign) => campaign.id === selectedCampaignId);
  ...
  const [previousDisasterRole, setPreviousDisasterRole] = useState('');
  ...
  useEffect(() => {
    if (!token) return;
    fetch('http://localhost:3001/api/v1/forms/my-applications', {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then((res) => res.ok ? res.json() : Promise.resolve({ data: [] }))
      .then((payload) => {
        const approved = (payload.data ?? [])
          .filter((app: any) => app.status === 'APPROVED' && app.raw?.volunteer_roles?.campaign_id)
          .map((app: any) => `${app.raw.volunteer_roles.campaign_id}:${String(app.raw.volunteer_roles.title ?? '').toLowerCase()}`);
        setApprovedRoleKeys(approved);
      })
      .catch(() => setApprovedRoleKeys([]));
  }, [token]);
  ...
  const roleSlot = (role: string) => selectedCampaign?.role_slots?.find((slot: any) => String(slot.title ?? '').toLowerCase() === role);
  const isRoleUnavailable = (role: string) => {
    if (!selectedCampaignId) return false;
    const alreadyApproved = approvedRoleKeys.includes(`${selectedCampaignId}:${role}`);
    return alreadyApproved || Boolean(roleSlot(role)?.is_full);
  };
  
  const handleNextToStep3 = () => {
    if (isSiteValid && isTimeValid && isRoleValid && selectedRole && !isRoleUnavailable(selectedRole) && isEmergencyContactValid && isCheckboxesValid) {
      setShowErrors(false);
      setStep(3);
    } else {
      setShowErrors(true);
  ...
  const handleSubmitFinal = async () => {
    ...
    if (isStep3Valid) {
      try {
        ...
        fd.append('emergency_contact_name', emergencyContactName.trim());
        fd.append('emergency_contact_number', emergencyContactNumber.trim());
        fd.append('emergency_contact', `${emergencyContactName.trim()} - ${emergencyContactNumber.trim()}`);
        
        // Exact mobile fields
        fd.append('disaster_experience', String(qDisaster));
        fd.append('previous_disaster_role', previousDisasterRole.trim());
        fd.append('rugged_environment', String(qRugged));
        fd.append('medical_conditions', String(qMedical));
        fd.append('medical_condition_details', medicalConditionDetails.trim());
            ...
            <View style={styles.twoColumnGrid}>
              ...
              <View style={[styles.leftColumnOutline, { zIndex: 50, flex: 1.5 }]}>
                <View>
                          ...
                          <ScrollView style={{ maxHeight: 200 }} showsVerticalScrollIndicator={true}>
                            {volunteerCampaigns.map((camp, index) => (
                              <Pressable key={index} style={styles.dropdownMenuItem} onPress={() => { setSelectedSite(camp.name); setSelectedCampaignId(camp.id); setIsSiteDropdownOpen(false); }}>
                                ...
                                {camp.participant_remaining !== null && camp.participant_remaining !== undefined && (
                                  <Text style={styles.dropdownMetaText}>{camp.participant_remaining} slot{camp.participant_remaining === 1 ? '' : 's'} remaining</Text>
                                )}
                                {camp.sitemanager_notes && (
                                  <Text style={styles.dropdownNoteText}>{camp.sitemanager_notes}</Text>
                                )}
                              </Pressable>
                            ))}
                  ...
                  {selectedCampaign && (
                    <View style={styles.selectedMissionInfo}>
                      {selectedCampaign.participant_remaining !== null && selectedCampaign.participant_remaining !== undefined && (
                        <Text style={styles.selectedMissionMeta}>
                          {selectedCampaign.participant_remaining} volunteer slot{selectedCampaign.participant_remaining === 1 ? '' : 's'} remaining
                        </Text>
                      )}
                      {selectedCampaign.sitemanager_notes && (
                        <Text style={styles.selectedMissionNote}>Site Manager Notes: {selectedCampaign.sitemanager_notes}</Text>
                      )}
                    </View>
                  )}

                  <RequiredLabel>Select Your Role</RequiredLabel>
                  <View style={styles.roleSelectionGroup}>
                    <Pressable disabled={isRoleUnavailable('medic')} style={[styles.roleSelectBox, selectedRole === 'medic' && styles.roleSelectBoxActive, isRoleUnavailable('medic') && styles.roleSelectBoxDisabled, showErrors && !isRoleValid && styles.errorBorder]} onPress={() => setSelectedRole('medic')}>
                      ...
                      {roleSlot('medic')?.slots_remaining !== null && roleSlot('medic')?.slots_remaining !== undefined && (
                        <Text style={styles.roleSlotText}>{isRoleUnavailable('medic') ? 'Unavailable' : `${roleSlot('medic')?.slots_remaining} slot${roleSlot('medic')?.slots_remaining === 1 ? '' : 's'}`}</Text>
                      )}
                    </Pressable>
                    <Pressable disabled={isRoleUnavailable('logistics')} style={[styles.roleSelectBox, selectedRole === 'logistics' && styles.roleSelectBoxActive, isRoleUnavailable('logistics') && styles.roleSelectBoxDisabled, showErrors && !isRoleValid && styles.errorBorder]} onPress={() => setSelectedRole('logistics')}>
                      ...
                      {roleSlot('logistics')?.slots_remaining !== null && roleSlot('logistics')?.slots_remaining !== undefined && (
                        <Text style={styles.roleSlotText}>{isRoleUnavailable('logistics') ? 'Unavailable' : `${roleSlot('logistics')?.slots_remaining} slot${roleSlot('logistics')?.slots_remaining === 1 ? '' : 's'}`}</Text>
                      )}
                    </Pressable>
                    <Pressable disabled={isRoleUnavailable('field')} style={[styles.roleSelectBox, selectedRole === 'field' && styles.roleSelectBoxActive, isRoleUnavailable('field') && styles.roleSelectBoxDisabled, showErrors && !isRoleValid && styles.errorBorder]} onPress={() => setSelectedRole('field')}>
                      ...
                      {roleSlot('field')?.slots_remaining !== null && roleSlot('field')?.slots_remaining !== undefined && (
                        <Text style={styles.roleSlotText}>{isRoleUnavailable('field') ? 'Unavailable' : `${roleSlot('field')?.slots_remaining} slot${roleSlot('field')?.slots_remaining === 1 ? '' : 's'}`}</Text>
                      )}
                    </Pressable>
                  </View>
                  {showErrors && !isRoleValid && <Text style={[styles.errorText, {marginTop: -10, marginBottom: 20}]}>• Role selection is required.</Text>}

                  <RequiredLabel style={{ marginTop: 10 }}>Required Document</RequiredLabel>
                  ...
                  <RequiredLabel style={{ marginTop: 18 }}>Emergency Contact Name</RequiredLabel>
                  <TextInput
                    style={[styles.textInputBox, showErrors && !isEmergencyContactValid && styles.errorBorder]}
                    placeholder="Full name"
                    placeholderTextColor="#9CA3AF"
                    value={emergencyContactName}
                    onChangeText={setEmergencyContactName}
                  />
                  <RequiredLabel style={{ marginTop: 12 }}>Emergency Contact Number</RequiredLabel>
                  <TextInput
                    style={[styles.textInputBox, showErrors && !isEmergencyContactValid && styles.errorBorder]}
                ...
                <View style={[styles.bottomAnchorBox, { marginTop: 20 }]}>
                  <Text style={[styles.sectionHeaderTitle, { marginBottom: 15 }]}>Checkbox for Vetting <Text style={styles.requiredAsterisk}>*</Text></Text>
                  ...
                  {showErrors && (!isSiteValid || !isTimeValid || !isRoleValid || !isEmergencyContactValid || !isCheckboxesValid) && (
                    <Text style={[styles.errorText, {marginTop: 20}]}>• Please check all required fields. Emergency contact number must be exactly 11 digits.</Text>
                  )}
                </View>
              </View>

              {/* Right Column: Next Button */}
              <View style={[styles.rightColumnOutline, { flex: 1, justifyContent: 'flex-end' }]}>
            ...
            <View style={styles.twoColumnGrid}>
              ...
              <View style={[styles.leftColumnOutline, { flex: 1.2 }]}>
                <View>
                  ...
                  {qDisaster === true && (
                    <TextInput
                      style={[styles.textInputBox, showErrors && previousDisasterRole.trim() === '' && styles.errorBorder, { marginBottom: 20 }]}
                  ...
                  <View style={styles.healthGreenBox}>
                    ...
                    {qMedical === true && (
                      <TextInput
                        style={[styles.textInputBox, showErrors && medicalConditionDetails.trim() === '' && styles.errorBorder, { marginBottom: 20 }]}
...
const styles = StyleSheet.create({
  ...
  requiredAsterisk: { color: '#EF4444', fontSize: 16, fontWeight: '800', position: 'relative', top: -4 } as any,
  ...
  dropdownNoteText: { fontSize: 12, color: '#6B7280', marginTop: 3, lineHeight: 18 },
  selectedMissionInfo: { backgroundColor: '#F8FAFC', borderWidth: 1, borderColor: '#E5E7EB', borderRadius: 8, padding: 12, marginTop: -12, marginBottom: 20 },
  selectedMissionMeta: { fontSize: 13, color: '#2563EB', fontWeight: '700', marginBottom: 4 },
  selectedMissionNote: { fontSize: 13, color: '#374151', lineHeight: 20 },
  ...
  roleSelectBoxDisabled: { opacity: 0.45, backgroundColor: '#F3F4F6' },
  ...
  roleSlotText: { fontSize: 11, color: '#6B7280', marginTop: 5, fontWeight: '700' },
