...
export default function PledgePage() {
  ...
  const updateItem = (index: number, field: keyof typeof items[0], value: string) => {
    ...
    if (field === 'qty') {
      // Strictly only numbers
      (newItems[index] as any)[field] = value.replace(/\D/g, '');
    } else {
  ...
  // An item is considered "active" (attempted) if any field has been touched/filled
  ...
  // All active items must be fully completed
  const allActiveItemsAreComplete = items.every(item => {
    const isAnyFilled = item.qty.trim() !== '' || item.name.trim() !== '' || item.unit.trim() !== '' || item.condition.trim() !== '';
    if (!isAnyFilled) return true; // Completely empty rows are fine
    return item.qty.trim() !== '' && item.name.trim() !== '' && item.unit.trim() !== '' && item.condition.trim() !== '';
  });

  const isItemsValid = hasActiveItems && allActiveItemsAreComplete;
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.summaryBox}>
              ...
              <Text style={styles.summaryLabel}>Donor Message:</Text>
              <Text style={styles.summaryValue}>{donorMessage.trim() || 'No donor message provided.'}</Text>
              {validItems.map((item, idx) => <Text key={idx} style={styles.summaryValue}>• {item.qty} {item.unit} of {item.name} ({item.condition})</Text>)}
            </View>
        ...
        <View style={styles.gridContainer}>
          ...
          <View style={styles.leftColumnOutline}>
            ...
            <View style={[styles.card, { zIndex: 50 }]}>
              ...
              <RequiredLabel>Select Campaign / Site Location</RequiredLabel>
                      ...
                      {campaigns.map((camp, index) => (
                        <Pressable key={index} style={(state: any) => [styles.dropdownMenuItem, styles.animated, state.hovered && { backgroundColor: '#F0FDF4', paddingLeft: 20 }]} onPress={() => { setSelectedSite(camp.name); setSelectedCampaignId(camp.id); setIsSiteDropdownOpen(false); if(showErrors) setShowErrors(false); }}>
                          ...
                          {camp.participant_remaining !== null && camp.participant_remaining !== undefined && (
                            <Text style={styles.dropdownMetaText}>{camp.participant_remaining} donor slot{camp.participant_remaining === 1 ? '' : 's'} remaining</Text>
                          )}
                          {camp.sitemanager_notes && (
                            <Text style={styles.dropdownNoteText}>Site Manager Notes: {camp.sitemanager_notes}</Text>
                          )}
                        </Pressable>
                      ))}
                      {campaigns.length === 0 && (
                ...
                <View style={styles.selectedMissionInfo}>
                  {selectedCampaign.participant_remaining !== null && selectedCampaign.participant_remaining !== undefined && (
                    <Text style={styles.selectedMissionMeta}>
                      {selectedCampaign.participant_remaining} donor slot{selectedCampaign.participant_remaining === 1 ? '' : 's'} remaining
                    </Text>
                  )}
          ...
          <View style={styles.rightColumnOutline}>
            <View style={[styles.card, { flex: 1 }]}>
              <Text style={styles.sectionHeaderTitle}>Donation Items <Text style={styles.requiredAsterisk}>*</Text></Text>
              ...
              <View style={styles.itemsOuterFrame}>
                <ScrollView style={styles.itemsScroll} showsVerticalScrollIndicator={true}>
                  {items.map((item, index) => {
                    const isRowActive = item.qty.trim() !== '' || item.name.trim() !== '' || item.unit.trim() !== '' || item.condition.trim() !== '';
                    const isFirstRowEmpty = !hasActiveItems && index === 0;
                    const showQtyError = showErrors && !isItemsValid && (isFirstRowEmpty || (isRowActive && item.qty.trim() === ''));
                    const showUnitError = showErrors && !isItemsValid && (isFirstRowEmpty || (isRowActive && item.unit.trim() === ''));
                    const showNameError = showErrors && !isItemsValid && (isFirstRowEmpty || (isRowActive && item.name.trim() === ''));
                    const showConditionError = showErrors && !isItemsValid && (isFirstRowEmpty || (isRowActive && item.condition.trim() === ''));

                    return (
                      <View key={index} style={[styles.itemRow, { zIndex: items.length - index }]}>
                        <TextInput 
                          style={[styles.qtyBox, styles.animated, showQtyError && styles.errorBorder, { width: 50 }]} 
                          value={item.qty} 
                        ...
                        <View style={{ width: 100, position: 'relative' }}>
                          <Pressable 
                            style={[styles.smallDropdown, showUnitError && styles.errorBorder, { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }]} 
                        ...
                        <TextInput 
                          style={[styles.nameBox, styles.animated, showNameError && styles.errorBorder]} 
                          value={item.name} 
                        ...
                        <View style={{ width: 135, position: 'relative' }}>
                          <Pressable 
                            style={[styles.smallDropdown, showConditionError && styles.errorBorder, { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }]} 
              ...
              {showErrors && !isItemsValid && (
                <Text style={[styles.errorText, {marginTop: 8}]}>
                  {!hasActiveItems 
                    ? '• At least one valid item is required.' 
                    : '• Please fill in all fields (Qty, Unit, Item Name, and Condition) for the items you want to donate.'}
                </Text>
              )}
              ...
              <Text style={[styles.inputLabel, { marginTop: 22 }]}>Donor Message</Text>
              <TextInput
                style={styles.messageBox}
                value={donorMessage}
                onChangeText={setDonorMessage}
                placeholder="Add notes, concerns, or a message for the site team"
                placeholderTextColor="#9CA3AF"
                multiline
              />
            </View>
...
const styles = StyleSheet.create({
  ...
  dropdownMetaText: { fontSize: 12, color: '#047857', fontWeight: '700', marginTop: 3 },
  dropdownNoteText: { fontSize: 12, color: '#6B7280', marginTop: 3, lineHeight: 18 },
  selectedMissionInfo: { backgroundColor: '#F8FAFC', borderWidth: 1, borderColor: '#E5E7EB', borderRadius: 8, padding: 12, marginTop: -10, marginBottom: 18 },
  selectedMissionMeta: { fontSize: 13, color: '#047857', fontWeight: '700', marginBottom: 4 },
  ...
  messageBox: { minHeight: 84, backgroundColor: '#FFFFFF', paddingVertical: 12, paddingHorizontal: 15, borderRadius: 8, borderWidth: 1, borderColor: '#D1D5DB', color: '#111827', fontSize: 14, textAlignVertical: 'top' } as any,
