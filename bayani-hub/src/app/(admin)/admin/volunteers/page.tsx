...
const VOLUNTEER_ROLES = ["All Applications", "Medic", "Logistics", "Field"];
...
export default function Volunteers() {
  ...
  useEffect(() => {
    const mapApplications = (rows: any[] = []): VolunteerApp[] => rows.map((row: any) => {
    ...
    async function fetchVolunteers() {
      setLoading(true);
      try {
        const queue = await apiFetch<any[]>("/applications");
        setVolunteers(mapApplications(queue));
      } catch (err) {
  ...
  const filteredVolunteers = useMemo(() => {
    ...
    return volunteers.filter((v) => v.role === selectedRole || (selectedRole === "Logistics" && v.role === "Logistic"));
  }, [selectedRole, volunteers]);

  // Collect unique roles from real data for filter buttons
  const uniqueRoles = useMemo(() => {
    const roles = new Set([...VOLUNTEER_ROLES, ...volunteers.map((v) => v.role)]);
      ...
      <main className={styles.main}>
        <div className={styles.pageHeader}>
          ...
          <div style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap", justifyContent: "flex-end" }}>
            <Link href='/admin/volunteer-history' className={styles.backButton}>
              Volunteer History
            </Link>
            <Link
              href='/admin/volunteer-verification'
              className={styles.backButton}
            >
              <svg className={styles.svg16} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
