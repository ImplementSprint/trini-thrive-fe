...
export function LoginForm() {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      document.cookie = "persona=end-user; path=/; max-age=86400";
      setIsSuccess(true);
      window.setTimeout(() => {
      ...
      <section className={styles.shell}>
        ...
        <div className={`${styles.card} ${isSuccess ? styles.cardSuccess : ""}`}>
          ...
          <p className={styles.footer}>
            Don&apos;t have an account? <Link className={isLoading || isSuccess ? styles.linkDisabled : ""} href="/enduser/signup" aria-disabled={isLoading || isSuccess}>Sign Up</Link>
          </p>
        </div>
      </section>
    </main>
