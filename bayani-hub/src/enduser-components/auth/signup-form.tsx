...
export function SignupForm() {
  ...
  const validate = () => {
    ...
    if (!form.birthDate.trim()) {
      ...
      nextErrors.birthDate = "Use a valid YYYY-MM-DD date.";
    }
          ...
          <form onSubmit={handleSubmit}>
            <div className={styles.grid}>
              <div className={styles.formColumn}>
                ...
                <label className={styles.field}>
                  <span className={styles.label}>Phone Number <span className={styles.required}>*</span></span>
                  <input
                    className={`${styles.input} ${errors.phone ? styles.inputError : ""}`}
                    value={form.phone}
                    onChange={(event) =>
                      setForm((current) => ({ ...current, phone: sanitizeDigits(event.target.value, 11) }))
                    }
                    type="tel"
                    inputMode="numeric"
                    maxLength={11}
                ...
                <label className={styles.field}>
                  <span className={styles.label}>Date of Birth <span className={styles.required}>*</span></span>
                  <input
                    className={`${styles.input} ${errors.birthDate ? styles.inputError : ""}`}
                    value={form.birthDate}
                    onChange={(event) =>
                      setForm((current) => ({ ...current, birthDate: normalizeDateInput(event.target.value) }))
                    }
              ...
              <div className={styles.verifyColumn}>
                ...
                <div className={styles.uploadCard}>
                  ...
                  <p className={styles.uploadSub}>Supported formats: JPG, PNG (Max 10MB)</p>
            ...
            <div className={styles.actions}>
              ...
              <p className={styles.footer}>
                Already have an account? <Link href="/enduser/login">Log In</Link>
              </p>
            </div>
          </form>
        </div>
